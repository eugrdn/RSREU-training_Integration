...
```
Здесь, в фукнкции meetQuery, проверяется, удовлетворяет ли объект "book" каждому из фильтров (строке поиска, категориям и фильтрам). Проверка по "фильтру" уже приведена. Проверку по строке поиска и категориям необходимо написать Вам самим.

#### Задание 3.1
>Написать функции проверки объекта книги на соответствие параметру строки поиска, параметру категории.
...
Необходимо написать функцию обработчик POST-запроса `/api/book` в зависимости от параметра `action`.
